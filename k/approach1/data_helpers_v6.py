...
def clean_str(string):
    ...
    string = re.sub(r"WIRED See the event \> Visit the Connective 2.0 \> ", " ", string)
    string = re.sub(r"Share this article Share via Email Share on Facebook Share on Twitter Share on Google\+", " ", string)
    string = re.sub(r"\[article_title\]\s\[article_body\]", " ", string)
    string = re.sub(r"News Entertainment Lifestyle Tech & Innovation All Sections", " ", string)
...
def text_preprocessing(x_text, MAX_NB_WORDS, MAX_LEN_DOC, SPLIT_INDEX, INSPECTION_ROW=4):
    ...
    x_train = tokenizer.texts_to_sequences(x_train) # might need to change if not in pd.df
    x_test = tokenizer.texts_to_sequences(x_test)
    x_text = np.concatenate((x_train, x_test), axis=0)
    # print (x_text[INSPECTION_ROW], len(x_text[INSPECTION_ROW]))

    # exmaple
    # print (tokenizer.texts_to_sequences(["choice mobbed tom weather test mother"]))

    print ("Padding...")
    x_text = sequence.pad_sequences(x_text, maxlen=MAX_LEN_DOC, dtype='int32',
                                   padding="pre", truncating="post")
    # print (x_text[INSPECTION_ROW])
...
def load_data(MAX_LEN_DOC=100, NUM_SUMPLE=5000, MAX_NB_WORDS=5000, 
    ...
    # print(pre_x.iloc[5])
    # print('numpy')
    # print(pre_x.values[5])
    assert (len(pre_x.iloc[5]) > 2)
...
def evaluation(y_pred, y_test, history, labelEncoder):
    print ('---------Confusion Matrix Report -------------------- \n')
    target_names = labelEncoder.inverse_transform([0, 1, 2, 3, 4, 5, 6, 7, 8])
